...
export function makeWASocket(connectionOptions, options = {}) {
    /**
     * @type {import('ourin-baileys').WASocket}
     */
    ...
    let sock = Object.defineProperties(conn, {
        ...
        sendFile: {
             ...
             * @param {import('ourin-baileys').proto.WebMessageInfo} quoted
             * @param {Boolean} ptt
            ...
            async value(jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) {
                ...
                /**
                 * @type {import('ourin-baileys').proto.WebMessageInfo}
                 */
                let m
                try {
...
N:;${name.replace(/\n/g, '\\n')};;;
FN:${name.replace(/\n/g, '\\n')}
TEL;type=CELL;type=VOICE;waid=${number}:${PhoneNumber('+' + number).number?.international || number}${biz.description ? `
...
sendButtonImg: {
    async value (jid, buffer, contentText, footerText, button1, id1, quoted, options) {
        await conn.sendFile(jid, buffer, '', contentText, quoted, false, options)
        const fullText = [contentText, `• ${button1}`, footerText ? `_${footerText}_` : ''].filter(Boolean).join('\n\n').trim()
        return conn.sendMessage(jid, { text: fullText, ...global.adReply, ...(options || {}) }, { quoted })
    }
},
send2ButtonImg: {
    async value (jid, buffer, contentText, footerText, button1, id1, button2, id2, quoted, options) {
        await conn.sendFile(jid, buffer, '', contentText, quoted, false, options)
        const fullText = [contentText, `• ${button1}\n• ${button2}`, footerText ? `_${footerText}_` : ''].filter(Boolean).join('\n\n').trim()
        return conn.sendMessage(jid, { text: fullText, ...global.adReply, ...(options || {}) }, { quoted })
    }
},
send3ButtonImg: {
    async value (jid, buffer, contentText, footerText, button1, id1, button2, id2, button3, id3, quoted, options) {
        await conn.sendFile(jid, buffer, '', contentText, quoted, false, options)
        const fullText = [contentText, `• ${button1}\n• ${button2}\n• ${button3}`, footerText ? `_${footerText}_` : ''].filter(Boolean).join('\n\n').trim()
        return conn.sendMessage(jid, { text: fullText, ...global.adReply, ...(options || {}) }, { quoted })
    }
},
        /** 
...
send3ButtonVid: {
    ...
    sendBut: {
        async value (jid, content, footerText, button1, id1, quoted, options) {
            const fullText = [content, `• ${button1}`, footerText ? `_${footerText}_` : ''].filter(Boolean).join('\n\n').trim()
            return conn.sendMessage(jid, { text: fullText, ...global.adReply, ...(options || {}) }, { quoted })
        }
    },
    send2Button: {
        async value (jid, content, footerText, button1, id1, button2, id2, quoted, options) {
            const fullText = [content, `• ${button1}\n• ${button2}`, footerText ? `_${footerText}_` : ''].filter(Boolean).join('\n\n').trim()
            return conn.sendMessage(jid, { text: fullText, ...global.adReply, ...(options || {}) }, { quoted })
        }
    },
    send3Button: {
        ...
        sendButton: {
            async value(jid, text = '', footer = '', buffer, buttons, quoted, options) {
                ...
                const btnText = buttons.map(b => `• ${Array.isArray(b) ? b[0] : b}`).join('\n')
                const fullText = [text, btnText, footer ? `_${footer}_` : ''].filter(Boolean).join('\n\n').trim()
                if (buffer) {
                    try { await conn.sendFile(jid, buffer, '', text, quoted, false, options) } catch {}
                }
                return conn.sendMessage(jid, { text: fullText, ...global.adReply, ...(options || {}) }, { quoted })
            },
            enumerable: true
    ...
    */
    send3TemplateButtonImg: {
        async value (jid, buffer, content, footerText, button1, id1, button2, id2, button3, id3, quoted, options) {
            await conn.sendFile(jid, buffer, '', content, quoted, false, options)
            const fullText = [content, `• ${button1}\n• ${button2}\n• ${button3}`, footerText ? `_${footerText}_` : ''].filter(Boolean).join('\n\n').trim()
            return conn.sendMessage(jid, { text: fullText, ...global.adReply, ...(options || {}) }, { quoted })
        }
    },
    sendTemplateButtonFakeImg: {
        async value (jid, buffer, content, footerText, btn1, id1, options) {
            await conn.sendFile(jid, buffer, '', content, null, false, options)
            const fullText = [content, `• ${btn1}`, footerText ? `_${footerText}_` : ''].filter(Boolean).join('\n\n').trim()
            return conn.sendMessage(jid, { text: fullText, ...global.adReply, ...(options || {}) }, {})
        }
    },
    send2TemplateButtonFakeImg: {
        async value (jid, buffer, content, footerText, btn1, id1, btn2, id2, quoted, options) {
            await conn.sendFile(jid, buffer, '', content, quoted, false, options)
            const fullText = [content, `• ${btn1}\n• ${btn2}`, footerText ? `_${footerText}_` : ''].filter(Boolean).join('\n\n').trim()
            return conn.sendMessage(jid, { text: fullText, ...global.adReply, ...(options || {}) }, { quoted })
        }
    },
    send3TemplateButtonFakeImg: {
        async value (jid, buffer, content, footerText, btn1, id1, btn2, id2, btn3, id3, quoted, options) {
            await conn.sendFile(jid, buffer, '', content, quoted, false, options)
            const fullText = [content, `• ${btn1}\n• ${btn2}\n• ${btn3}`, footerText ? `_${footerText}_` : ''].filter(Boolean).join('\n\n').trim()
            return conn.sendMessage(jid, { text: fullText, ...global.adReply, ...(options || {}) }, { quoted })
        }
    },
...
send3TemplateButtonLoc: {
        ...
        sendButtonLoc: {
                        async value (jid, buffer, content, footer, button1, row1, quoted, options = {}) {
                            const fullText = [content, `• ${button1}`, footer ? `_${footer}_` : ''].filter(Boolean).join('\n\n').trim()
                            return conn.sendMessage(jid, { text: fullText, ...global.adReply, ...(options || {}) }, { quoted })
                        }
                        },
                      send2ButtonLoc: {
                        async value (jid, buffer, content, footer, button1, row1, button2, row2, quoted, options = {}) {
                            const fullText = [content, `• ${button1}\n• ${button2}`, footer ? `_${footer}_` : ''].filter(Boolean).join('\n\n').trim()
                            return conn.sendMessage(jid, { text: fullText, ...global.adReply, ...(options || {}) }, { quoted })
                        }
                        },
                        ...
                        send3ButtonLoc: {
                            ...
                            const fullText = [content, `• ${button1}\n• ${button2}\n• ${button3}`, footer ? `_${footer}_` : ''].filter(Boolean).join('\n\n').trim()
                            return conn.sendMessage(jid, { text: fullText, ...global.adReply, ...(options || {}) }, { quoted })
                        }
                        },
        sendHydrated: {
            async value(jid, text = '', footer = '', buffer, url, urlText, call, callText, buttons, quoted, options) {
                ...
                if (url || urlText) {
                    ...
                    urls.forEach((u, i) => btnLines.push(`• ${texts[i] || u}`))
                }
                if (call || callText) {
                    const calls = Array.isArray(call) ? call : [call]
                    const texts = Array.isArray(callText) ? callText : [callText]
                    calls.forEach((c, i) => btnLines.push(`• ${texts[i] || c}`))
                }
                if (buttons && buttons.length) {
                    if (!Array.isArray(buttons[0])) buttons = [buttons]
                    buttons.forEach(([t]) => btnLines.push(`• ${t}`))
                }
                const fullText = [text, btnLines.join('\n'), footer ? `_${footer}_` : ''].filter(Boolean).join('\n\n').trim()
                if (buffer) {
                    try { await conn.sendFile(jid, buffer, '', text, quoted, false, options) } catch {}
                }
                return conn.sendMessage(jid, { text: fullText, ...global.adReply, ...(options || {}) }, { quoted })
        ...
        sendHydrated2: {
            async value(jid, text = '', footer = '', buffer, url, urlText, url2, urlText2, buttons, quoted, options) {
                ...
                if (url2 || urlText2) {
                    const urls2 = Array.isArray(url2) ? url2 : [url2]
                    const texts2 = Array.isArray(urlText2) ? urlText2 : [urlText2]
                    urls2.forEach((u, i) => btnLines.push(`• ${texts2[i] || u}`))
                }
                if (buttons && buttons.length) {
                    if (!Array.isArray(buttons[0])) buttons = [buttons]
                    buttons.forEach(([t]) => btnLines.push(`• ${t}`))
...
msToDate: {
   async value (ms) {
        ...
        cMod: {
             ...
             * cMod
             * @param {String} jid 
             * @param {import('ourin-baileys').proto.WebMessageInfo} message 
             * @param {String} text 
             * @param {String} sender 
             * @param {*} options 
             * @returns 
        ...
        fakeReply: {
             ...
             */
            value(jid, text = '', fakeJid = this.user.id, fakeText = '', fakeGroupJid, options) {
        ...
        getName: {
            ...
            value(jid = '', withoutContact = false) {
                ...
                if (!jid) return Promise.resolve('Unknown')
                withoutContact = conn.withoutContact || withoutContact
                let v
                if (jid.endsWith('@g.us')) return new Promise(async (resolve) => {
                    v = conn.chats[jid] || {}
                    if (!(v.name || v.subject)) v = await conn.groupMetadata(jid) || {}
                    resolve(v.name || v.subject || PhoneNumber('+' + jid.replace('@s.whatsapp.net', '')).number?.international || jid)
                })
                    ...
                    conn.user :
                    (conn.chats[jid] || {})
                return (withoutContact ? '' : v.name) || v.subject || v.vname || v.notify || v.verifiedName || PhoneNumber('+' + jid.replace('@s.whatsapp.net', '')).number?.international || jid
            },
            enumerable: true
        },
        loadMessage: {
             ...
             * @returns {import('ourin-baileys').proto.WebMessageInfo}
             */
            value(messageID) {
        ...
        sendUrlButton: {
            async value (jid, content, distek, link, retek, id, quoted) {
                const fullText = [content, `• ${distek}: ${link}`, `• ${retek}`].filter(Boolean).join('\n\n').trim()
                return conn.sendMessage(jid, { text: fullText, ...global.adReply }, { quoted })
            }
        },
        sendGroupV4Invite: {
            /**
        ...
        processMessageStubType: {
             ...
             * @param {import('ourin-baileys').proto.WebMessageInfo} m 
             */
            async value(m) {
        ...
        sendStimg: {
            async value (jid, path, quoted, options = {}) {
                    let buff = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,`[1], 'base64') : /^https?:\/\//.test(path) ? Buffer.from(await (await fetch(path)).arrayBuffer()) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
                    let buffer
        ...
        sendImageAsSticker: {
            async value(jid, path, quoted, options = {}) {
                return conn.sendStimg(jid, path, quoted, options)
            },
            enumerable: true
        },
        sendVideoAsSticker: {
            async value(jid, path, quoted, options = {}) {
                return conn.sendStvid(jid, path, quoted, options)
            },
            enumerable: true
        },
        pushMessage: {
                ...
                for (const message of m) {
                    try {
                        ...
                        if (message.message?.[mtype]?.contextInfo?.quotedMessage) {
                            /**
                             * @type {import('ourin-baileys').proto.IContextInfo}
                             */
                            let context = message.message[mtype].contextInfo
                            let participant = conn.decodeJid(context.participant)
                            const remoteJid = conn.decodeJid(context.remoteJid || participant)
                            /**
                             * @type {import('ourin-baileys').proto.IMessage}
                             * 
                             */
                            ...
                            if ((remoteJid && remoteJid !== 'status@broadcast') && quoted) {
                                ...
                                const qM = {
                                    key: {
                                        remoteJid,
                                        fromMe: areJidsSameUser(conn.user.id, remoteJid),
                                        id: context.stanzaId,
                                        participant,
                                    },
...
async value (status) {
        ...
        ...(typeof conn.chatRead !== 'function' ? {
            chatRead: {
                value(jid, participant = conn.user.id, messageID) {
...
export function smsg(conn, m, hasParent) {
    ...
    // fromObject does not always return a true instance — set prototype directly instead
    if (!(m instanceof M)) Object.setPrototypeOf(m, M.prototype)
    m.conn = conn
...
export function serialize() {
    ...
    return Object.defineProperties(proto.WebMessageInfo.prototype, {
        ...
        isBaileys: {
            get() {
                return (this.id?.startsWith('3EB0') && (this.id?.length === 12 || this.id?.length === 16)) || false
            }
        },
        chat: {
            get() {
                ...
                // Convert @lid DM JID to @s.whatsapp.net so bot can reply
        ...
        sender: {
            get() {
                const rawSender = this.conn?.decodeJid(this.key?.fromMe && this.conn?.user.id || this.participant || this.key.participant || this.chat || '')
                // Convert @lid to @s.whatsapp.net for database compatibility with Baileys v7
                if (rawSender && rawSender.endsWith('@lid')) {
                    const user = rawSender.split('@')[0]
                    return `${user}@s.whatsapp.net`
                }
                // Keep special jids as-is (@newsletter, @g.us, @broadcast, etc)
                if (rawSender && (rawSender.endsWith('@newsletter') || rawSender.endsWith('@broadcast'))) {
                    return rawSender
                }
        ...
        quoted: {
            get() {
                ...
                return Object.defineProperties(JSON.parse(JSON.stringify(typeof q === 'string' ? { text: q } : q)), {
                    ...
                    sender: {
                        get() {
                            const rawSender = (contextInfo.participant || this.chat || '').decodeJid()
                            // Convert @lid to @s.whatsapp.net for compatibility
                            if (rawSender && rawSender.endsWith('@lid')) {
                                return `${rawSender.split('@')[0]}@s.whatsapp.net`
                            }
                            return rawSender
                        },
                        enumerable: true
                    ...
                    mentionedJid: {
                        get() {
                            ...
                            // Normalize @lid to @s.whatsapp.net for compatibility
        ...
        mentionedJid: {
            get() {
                if (!this.msg) return []
                const mentions = this.msg.contextInfo?.mentionedJid || []
                // Normalize @lid to @s.whatsapp.net for compatibility
                return mentions.map(jid => {
                    if (jid && jid.endsWith('@lid')) {
                        return `${jid.split('@')[0]}@s.whatsapp.net`
